...
def parse_dollar_amount(x, min_valid=30000, max_valid=300000) -> int:
    """
    ...
    if amounts:
        ...
        if np.isinf(average_amount) or np.isnan(average_amount):
            return np.nan 
        elif average_amount > 10 and average_amount < 150:
            return int(round(average_amount))*1000
        elif average_amount < min_valid or average_amount > max_valid:
            return np.nan
