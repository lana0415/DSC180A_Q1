...
class DatabaseTarget(luigi.Target):
    ...
    def exists(self):
        ...
        cursor.execute(
            ...
            WHERE model_name = ? AND trial = ? AND race = ? AND gender = ? AND income = ? AND interest = ?
            ''',
            (
                self.model_name,
                self.trial,
...
class RunExperimentTask(luigi.Task):
    ...
    def run(self):
        ...
        data = (
            ...
            self.condition['race'], self.condition['gender'], self.condition['income'], self.condition['interest']
        )
